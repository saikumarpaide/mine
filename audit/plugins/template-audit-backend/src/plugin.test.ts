...
import { templateAuditPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [templateAuditPlugin],
    });

    await request(server).get('/api/template-audit/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/template-audit/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/template-audit/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/template-audit/todos/${createdTodoItem.id}`)
